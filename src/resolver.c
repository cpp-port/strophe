                        ...
                        size_t target_len, unsigned short *port)
{
    ...
    snprintf(sz, sizeof(sz), "res_query len %d", len);

    __android_log_print(ANDROID_LOG_INFO, "xmpp_resolver", sz);
    ...
    if (set > 0)
    {
       ...
       snprintf(sz, sizeof(sz), "resolver_srv_lookup_buf result %d", set);
